...
def get_categories(search_category):
    try:
        ...
        while True:
            # Results only contains basic playlist information.
            results = sp.category_playlists(category_id=search_category, limit=query_limit, country='US',
                                            offset=new_offset)

            for item in results['playlists']['items']:
                if item is not None and item['name'] is not None:
                    # Ex) ['https:', '', 'api.spotify.com', 'v1', 'playlists', 'id', 'tracks']
