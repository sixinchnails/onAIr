...
def recommendation():
    ...
    query = f"SELECT cluster, " + " + ".join([f"feat_{i} * {result[i]}" for i in range(7)]) + \
    ...
    for hive_row in cursor:
        ...
        cosine_similarity = 1 - distance.cosine(result, hive_features)

        # Append the (row, cosine_similarity) pair to the list
        closest_rows.append((hive_row[0], cosine_similarity * 0.7 + hive_row[2] * 0.3))
    ...
    top_fifty_closest_rows = closest_rows[:10]
